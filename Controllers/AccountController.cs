        ...
        public async Task<ActionResult> Register([Bind(Exclude = "UserPhoto")] RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (!await roleManager.RoleExistsAsync(RoleName.AdminRole))
                    {
                        await roleManager.CreateAsync(new IdentityRole(RoleName.AdminRole));
                        
                    }
                    if (!await roleManager.RoleExistsAsync(RoleName.UserRole))
                    {
